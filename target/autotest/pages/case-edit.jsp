...
<html class="x-admin-sm">
    ...
    String need_login = request.getParameter("need_login");
    String loginaddress = request.getParameter("need_loginadd");
...
<%--                    是否需要登陆--%>
                    <div class="layui-form-item">
                        <label for="need_login" class="layui-form-label">
                            <span class="x-red">*</span>是否需要登陆
                        </label>
                        <div class="layui-input-inline">
                            <select id="need_login" name="need_login" lay-filter="needlogin">
...
<%--                    <div class="layui-form-item" id="need_loginaddress">--%>
<%--                        <label for="loginaddress" class="layui-form-label">--%>
<%--                            <span class="x-red">*</span>登陆地址--%>
<%--                        </label>--%>
<%--                        <div class="layui-input-inline">--%>
<%--                            <input type="text" id="loginaddress" name="loginaddress" required="" lay-verify="required"--%>
<%--                                   autocomplete="off" class="layui-input" value=<%=loginaddress%>>--%>
<%--                        </div>--%>
<%--                        <div class="layui-form-mid layui-word-aux">--%>
<%--                            <span class="x-red">*</span>如需要登陆，请填写--%>
<%--                        </div>--%>
<%--                    </div>--%>

<%--                    登陆参数--%>

<%--                    <div class="layui-form-item" id="need_loginpar">--%>
<%--                        <label for="loginpar" class="layui-form-label">--%>
<%--                            <span class="x-red">*</span>登陆参数--%>
<%--                        </label>--%>
<%--                        <div class="layui-input-inline">--%>
<%--                            <input type="text" id="loginpar" name="loginpar" required="" lay-verify="required"--%>
<%--                                   autocomplete="off" class="layui-input" value=<%=loginpar%>>--%>
<%--                        </div>--%>
<%--                        <div class="layui-form-mid layui-word-aux">--%>
<%--                            <span class="x-red">*</span>请认证参照之前格式填写--%>
<%--                        </div>--%>
<%--                    </div>--%>
<%--请求参数--%>
                    <div class="layui-form-item">
        ...
        <script>
            layui.use(['form'], function() {
                form=layui.form;
                form.on('select(needlogin)', function(data){
                    var val=$('#need_login').val();
                    if(val=="是"){
                        $("#need_loginaddress").removeAttr("style");
                        $("#need_loginpar").removeAttr("style");
                    }else if(val=="否"){
                        $("#need_loginaddress").attr("style","display:none");
                        $("#need_loginpar").attr("style","display:none");
                    }else {
                        alert("非法参数");
                        xadmin.close();
                    }
        ...
        <script >
            //重写need_login
            var need_login=GetQueryValue('need_login')
            $('#need_login').append(new Option(need_login))
            if(need_login=='是'){
                $('#need_login').append(new Option('否'))
            }else if(need_login=='否'){
                $('#need_login').append(new Option('是'))
            }
        </script>
        <script >
            //重写status
...
//------------------------------------------------------------------------------
                                    $('#L_repass').click(function () {
                                        ...
                                        layer.msg("1111111111111")
                                        console.log("给父窗口传值")
                                        parent.layer.msg("您成功将标记传递给了父窗口："+val)
...
//------------------------------------------------------------------------------
                        ...
                        $.ajax({
                            type: "POST",//方法类型
                            dataType: "json",//预期服务器返回的数据类型
                            url: "/case/findAllCasePage" ,//url
                            success:function (data) {
                                console.log(data)
                                console.log("第二个ajax也执行了")
                            }
                        })
